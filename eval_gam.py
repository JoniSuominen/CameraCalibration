import numpy as np
from preprocessing import GAMOptimizer
import colour
...
def compute_error_sfu():
    ...
    response_human_macbeth =  compute_response(macbeth, D65, cmfs) * 2
    response_human_sfu = compute_response(test_sfu, D65, cmfs)
    
    response_sensor_macbeth = compute_response(macbeth, D65, ss, wb=True) * 2
    ...
    #lams = [lam] * 3
    #linGam = LinearGAM(terms=te(0,1,2, spline_order=2))
    #linGam.gridsearch(X_train, y_train[:, 0], lam=lams)
    #linGam.summary()
   ...
   # print(linGam.terms)
    ...
    Gam = GAMOptimizer(lams=0.00204081632653)

    Gam.fit(X_train, y_train) 

    pred(Gam, X_test, y_test, "DeltaE GAM, SFU")
    pred(Gam,response_sensor_macbeth, response_human_macbeth, "DeltaE GAM, MACBETH")
        
    macbeth_out_srgb = colour.XYZ_to_sRGB(response_human_macbeth)
    response_sensor_macbeth =  colour.XYZ_to_sRGB(Gam.predict(response_sensor_macbeth))
